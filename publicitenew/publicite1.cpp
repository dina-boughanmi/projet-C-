...
QSqlQueryModel *publicite1 ::afficher_inv()
{
    QSqlQueryModel *model = new QSqlQueryModel();
    model->setQuery("SELECT * FROM invite");
    model->setHeaderData(0,Qt::Horizontal,QObject::tr("ID_I"));
...
QSqlQueryModel * publicite1::jeux_concour(int NOMBRE)
{

     QSqlQuery query;

     query.prepare("SELECT *  FROM INVITE where ID_I=:NOMBRE");

       query.bindValue(":ID_I",NOMBRE);
       query.exec();
